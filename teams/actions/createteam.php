...
<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        $row = mysqli_query($con,$sql)->fetch_assoc();
        $team_id=$row['team_id'];
        
        $sql = "INSERT INTO `teamadmin`(`team_id`, `user_id`) VALUES ('$team_id','$userid')";
        $result=mysqli_query($con,$sql);

        $sql = "INSERT INTO `teammembers`(`team_id`, `user_id`) VALUES ('$team_id','$userid')";
        $result=mysqli_query($con,$sql);

        header("location: ..\\teamshome.php");
        echo"task enterd";
...
<section class="text-gray-600 body-font  w-3/4 mx-auto pt-20 ">
  <div class="container   mx-auto flex flex-wrap items-center">
    ...
    <div class=" bg-white rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0  shadow-lg">
      <h2 class="text-black text-lg text-center font-medium title-font mb-5">Add Team</h2>
      
      <form  action="createteam.php" method="post">
        <div class="relative mb-4">
            <label for="user-name" class="leading-7 text-sm text-black">Team Name</label>
